...
=end
class TextAnalyzer
  # we just want to read the file, so 'r' will be included
  def process
    file = File.open('sample_text.txt', 'r') 
    yield(file.read)  # we need to load the file's content so to pass the text content
    file.close
  end
end

=begin
  split the text at paragraphs, putting each segment into new array as own element.
  get a count of those elements
  output to screen a string that tells us the count
  - split the text at end of line break
  - split the text at the white space
=end

analyzer = TextAnalyzer.new
  # the read file will be assigned to block variable.
  # since we are working with a string, "\n" (return character) is our target. 
  # double return gives us a paragraph, single return gives us a new line.
